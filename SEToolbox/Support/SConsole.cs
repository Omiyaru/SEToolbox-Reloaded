    ...
    public class SConsole
    {
        ...
        public static void WriteLine(params object[] values)
        {
            ...
            if (_isAttached)
            {
                if (isDebugEvent)
                {
                    var traces = values.Select(v => DebugEvent.GetDebugTrace(v as string));
                    foreach (var trace in traces)
                    {
                        redirector.WriteLine(trace);
                    }
                }
                else
                {
                    var strings = values.Select(v => v?.ToString()).Where(s => s != null);
                    if (strings.Any())
                    {
                        redirector.WriteLine(string.Join(Environment.NewLine, strings));
                    }
                }
            }
        }
